        ...
        try:
            ...
            try:
                response = (
                    retry_func(
                        lambda: session.get(
                            subscribe_url, proxies=config.get_local_proxy, timeout=config.request_timeout
                        ),
                        name=subscribe_url,
                    )
                    if retry
                    else session.get(subscribe_url, proxies=config.get_local_proxy, timeout=config.request_timeout)
                )
